...
function EditFields({ id, weight, date, onSuccess }) {
  ...
  useEffect(() => {
    reset({ weight: weight || "", date: formattedDate || "" });
  ...
  return (
    <form
      ...
      className="flex flex-col gap-4 w-full max-w-sm p-3 sm:p-4 bg-white rounded-2xl"
    >
      {/* Weight Field */}
      <div className="flex flex-col sm:flex-row items-center gap-2 w-full">
        <label className="min-w-[70px] text-gray-700 font-medium">
          Weight:
        </label>
        <input
          type="number"
